...
test("Wirkungs-Engine trennt Zeitpfad, Unsicherheit und unberechnete Bereiche", async ({ page }, testInfo) => {
  ...
  await expect(page.getByTestId("effect-timeline")).toBeVisible({ timeout: 30_000 });
  await expect(page.getByTestId("effect-timeline").getByRole("row")).toHaveCount(7, { timeout: 30_000 });

  await page.getByRole("button", { name: "Langfristig", exact: true }).click();
  await page.getByRole("button", { name: "20 J.", exact: true }).click();
  await expect(page.getByTestId("effect-timeline").getByRole("row")).toHaveCount(22, { timeout: 30_000 });
