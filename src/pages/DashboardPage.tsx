    ...
    <>
      <section className="sticky-kpis" aria-label="Haushaltsübersicht">
        <div className="content-width kpi-grid">
          <KpiCard label="Einnahmen" value={fmtBn(revenueTotal)} delta={fmtDiff(revenueTotal - revenueStatusQuo)} tone={revenueTotal >= revenueStatusQuo ? "positive" : "negative"} onSource={() => onOpenSource("source-budget", fmtBn(revenueTotal))} />
          <KpiCard label="Ausgaben" value={fmtBn(expenseTotal)} delta={fmtDiff(expenseTotal - expenseStatusQuo)} tone="neutral" onSource={() => onOpenSource("source-budget", fmtBn(expenseTotal))} />
          <KpiCard label="Saldo" value={fmtBn(balance)} delta={fmtDiff(balance - balanceStatusQuo)} tone={balance >= balanceStatusQuo ? "positive" : "negative"} onSource={() => onOpenSource("source-budget", fmtBn(balance))} />
          <KpiCard label="Schuldenstand" value="2.634,0 Mrd. €" delta={fmtDiff(Math.max(0, -(balance - balanceStatusQuo)))} tone="warning" onSource={() => onOpenSource("source-budget", "2.634,0 Mrd. €")} />
        </div>
      </section>

      <main className="content-width dashboard-main">
        <div className="desktop-dashboard">
          <LinePanel title="Einnahmen" total={revenueTotal} items={revenues} onOpenSource={onOpenSource} onInteractive={onNavigateIncomeTax} />
          ...
          <LinePanel title="Ausgaben" total={expenseTotal} items={expenses} expense onOpenSource={onOpenSource} />
        </div>
        ...
        <div className="mobile-dashboard">
          ...
          <div className="mobile-tab-content">
            {mobileTab === "steuern" && <LinePanel title="Einnahmen" total={revenueTotal} items={revenues} onOpenSource={onOpenSource} onInteractive={onNavigateIncomeTax} />}
            ...
            {mobileTab === "ausgaben" && <LinePanel title="Ausgaben" total={expenseTotal} items={expenses} expense onOpenSource={onOpenSource} />}
...
function LinePanel({ title, total, items, expense = false, onOpenSource, onInteractive }: { title: string; total: number; items: Array<RevenueLine | ExpenseLine>; expense?: boolean; onOpenSource: (sourceId: string, value?: string) => void; onInteractive?: () => void }) {
  const statusQuo = items.reduce((sum, item) => sum + item.statusQuo, 0);
  return (
    <aside className="card-flat line-panel" aria-label={title}>
      <header>
        <div className="panel-title-row"><h2>{title}</h2><SourceButton onClick={() => onOpenSource("source-budget", fmtBn(total))} /></div>
      ...
      <ul>
        {items.map((item) => (
          <LineRow key={item.id} item={item} expense={expense} onClick={item.id === "est" ? onInteractive : undefined} onSource={() => onOpenSource(item.id === "est" ? "source-est" : "source-budget", fmtBn(item.value))} />
        ))}
      </ul>
    </aside>
  );
...
function CenterPanel({ tab, setTab, incomeTaxDelta, onOpenSource, onNavigateComparison, scenarios, onLoadScenario, onDeleteScenario }: { tab: Tab; setTab: (tab: Tab) => void; incomeTaxDelta: number; onOpenSource: (sourceId: string, value?: string) => void; onNavigateComparison: () => void; scenarios: ScenarioState[]; onLoadScenario: (scenario: ScenarioState) => void; onDeleteScenario: (scenario: ScenarioState) => void }) {
  return (
    <section className="center-panel">
      <div className="card-flat result-card">
        ...
        <div className="result-content">
          ...
          {tab === "haushalte" && <HouseholdsTab />}
          {tab === "verteilung" && <DistributionTab />}
          {tab === "regionen" && <RegionsTab />}
          {tab === "zeit" && <TimeTab delta={incomeTaxDelta} />}
        </div>
      </div>
...
function BudgetTab({ delta, onOpenSource }: { delta: number; onOpenSource: (sourceId: string, value?: string) => void }) {
  const effect = Math.abs(delta);
  return (
    <div className="budget-layout">
      <div>
        <div className="section-title"><div><h3>Wasserfall Budgetsaldo</h3><p>Veränderung gegenüber Status quo. Modellstufe: mit Verhaltenseffekt · Bandbreite ± 25 %.</p></div><SourceButton onClick={() => onOpenSource("source-est", fmtDiff(delta))} /></div>
...
function HouseholdsTab() {
  return <div><h3>Beispielhaushalte</h3><p className="section-description">Wirkung pro Monat im Reformszenario. Werte sind Modellrechnungen mit Bandbreite.</p><div className="household-grid">{households.map((household) => <article className="household-card" key={household.name}><div><strong>{household.name}</strong><small>{household.income}</small></div><em className={household.delta >= 0 ? "positive" : "negative"}>{household.delta >= 0 ? "+" : "−"}{Math.abs(household.delta)} € / Monat</em><p>Dieser Haushalt hat geschätzt {Math.abs(household.delta)} Euro {household.delta >= 0 ? "mehr" : "weniger"} pro Monat, weil er {household.reason}.</p><span>Bandbreite ± 15 %</span></article>)}</div></div>;
}

function DistributionTab() {
  const max = 120;
  return <div><h3>Nettowirkung nach Einkommensdezilen</h3><p className="section-description">Positive Werte bedeuten mehr verfügbares Haushaltseinkommen.</p><div className="distribution-bars">{deciles.map((item) => <div key={item.d}><span>{item.d}</span><i><b className={item.reform >= 0 ? "positive" : "negative"} style={{ left: item.reform >= 0 ? "50%" : `${50 - Math.abs(item.reform) / max * 50}%`, width: `${Math.abs(item.reform) / max * 50}%` }} /></i><em className={item.reform >= 0 ? "positive" : "negative"}>{item.reform >= 0 ? "+" : "−"}{Math.abs(item.reform)} €</em></div>)}</div></div>;
}

function RegionsTab() {
  const regions = ["Schleswig-Holstein", "Niedersachsen", "Nordrhein-Westfalen", "Hessen", "Sachsen", "Bayern", "Berlin", "Baden-Württemberg"];
  return <div><h3>Regionale Wirkung</h3><p className="section-description">Demodarstellung nach Bundesländern; regionale Daten werden in einem späteren Milestone angebunden.</p><div className="region-layout"><div className="germany-map" aria-label="Stilisierte Deutschlandkarte"><span>SH</span><span>NI</span><span>NW</span><span>HE</span><span>SN</span><span>BY</span><span>BE</span><span>BW</span></div><ul>{regions.map((region, index) => <li key={region}><span>{region}</span><strong>+{8 + index * 3} € / Monat</strong></li>)}</ul></div></div>;
}

function TimeTab({ delta }: { delta: number }) {
  return <div><h3>Zeitverlauf</h3><p className="section-description">Schematische Entwicklung der jährlichen Budgetwirkung.</p><div className="time-chart">{[2026, 2027, 2028, 2029, 2030, 2031].map((year, index) => <div key={year}><span>{(delta * (1 + index * 0.04)).toLocaleString("de-DE", { maximumFractionDigits: 1 })}</span><i style={{ height: `${60 + index * 15}px` }} /><small>{year}</small></div>)}</div></div>;
}

function MigrationCard({ onOpenSource }: { onOpenSource: (sourceId: string, value?: string) => void }) {
  return <article className="card-flat migration-card"><div className="section-title"><div><h3>Migration und Asyl · getrennte Teilbereiche</h3><p>Unterbringung, Verfahren, Integration, Bildung und Erwerbstätigkeit werden nicht zu einer scheinbar exakten Einzelzahl vermischt.</p></div><SourceButton onClick={() => onOpenSource("source-budget", "29,8 Mrd. €")} /></div><div className="migration-grid"><span><b>9,8</b> Unterbringung</span><span><b>5,1</b> Verfahren</span><span><b>4,4</b> Integration</span><span><b>10,5</b> weitere Bereiche</span></div></article>;
}
