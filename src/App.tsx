...
function readRoute(): AppRoute {
  ...
  return (["/", "/dashboard", "/einkommensteuer", "/einnahmen", "/ausgaben", "/vergleich", "/transparenz"] as AppRoute[]).includes(path as AppRoute) ? path as AppRoute : "/dashboard";
}
...
export function App() {
  ...
  const taxResult = useMemo(() => estimateIncomeTaxRevenue(scenario.incomeTax, scenario.modelLevel), [scenario.incomeTax, scenario.modelLevel]);
  ...
  const materializedScenario = useMemo<ScenarioDraft>(() => ({
    ...
    sourceIds: Array.from(new Set([...scenario.sourceIds, "source-revenue-model", "source-expense-model"])),
  ...
  useEffect(() => {
    ...
    void Promise.all([localServer.listSources(), localServer.listMetrics(), localServer.listScenarios(), localServer.getActiveDraft()]).then(([loadedSources, loadedMetrics, loadedScenarios, draft]) => {
      if (cancelled) return;
      setSources(loadedSources); setMetrics(loadedMetrics); setScenarios(loadedScenarios);
      if (draft) { dispatch({ type: "replace", scenario: normalizeScenarioDraft(draft.scenario) }); setActiveScenarioId(draft.activeScenarioId); }
      setReady(true);
    });
  ...
  const openSource = (metricOrSourceId: string, value?: string) => { const metric = metrics.find((item) => item.id === (legacyMetricIds[metricOrSourceId] ?? metricOrSourceId)); if (metric) setDrawer({ metric, value: value ?? metricValues[metric.id] }); };
  ...
  const loadScenario = (saved: ScenarioState) => { dispatch({ type: "replace", scenario: normalizeScenarioDraft(saved) }); setActiveScenarioId(saved.id); navigate("/dashboard"); showNotice(`„${saved.name}“ geladen`); };
  ...
  const newScenario = () => { dispatch({ type: "replace", scenario: { ...defaultScenarioDraft, name: "Neues Szenario" } }); setActiveScenarioId(null); showNotice("Neuer lokaler Entwurf angelegt"); };
  const duplicateScenario = async () => {
    const now = new Date().toISOString(); const duplicate: ScenarioState = { ...materializedScenario, id: crypto.randomUUID(), name: `${materializedScenario.name} (Kopie)`, createdAt: now, updatedAt: now };
    await localServer.saveScenario(duplicate); dispatch({ type: "replace", scenario: duplicate }); setActiveScenarioId(duplicate.id); await refreshScenarios(); showNotice("Szenario dupliziert");
  };
  ...
  const importScenario = async (file: File) => { try { dispatch({ type: "replace", scenario: scenarioFromJson(await file.text()) }); setActiveScenarioId(null); showNotice("Szenario importiert und als Entwurf geladen"); } catch (error) { showNotice(error instanceof Error ? error.message : "Szenario konnte nicht importiert werden"); } };
  ...
  return <div className="app-shell">
    ...
    {route === "/dashboard" && <DashboardPage incomeTaxResult={taxResult} revenueModuleResults={revenueModuleResults} expenseModuleResults={expenseModuleResults} metrics={metrics} sources={sources} scenarios={scenarios} onNavigateIncomeTax={() => navigate("/einkommensteuer")} onNavigateRevenue={navigateRevenueModule} onNavigateExpense={navigateExpenseModule} onNavigateComparison={() => navigate("/vergleich")} onOpenSource={openSource} onLoadScenario={loadScenario} onDeleteScenario={(saved) => void deleteScenario(saved)} />}
