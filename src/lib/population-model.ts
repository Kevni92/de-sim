...
export const populationSources = [
  {
    ...
    method: "Gerundete Alters- und Länderanteile bilden Kalibrierungsziele der synthetischen Baseline.",
  ...
  },
  {
    ...
    limitations: ["Vermögenswerte sind keine amtlichen Einzelwerte.", "Obere Vermögensränder sind besonders unsicher."],
    checkedAt: "2026-07-13",
  },
  {
...
const STATE_TARGETS: Record<string, number> = {
  ...
  "Bayern": 0.160,
  "Baden-Württemberg": 0.134,
  "Niedersachsen": 0.098,
  "Hessen": 0.076,
  "Rheinland-Pfalz": 0.050,
  "Sachsen": 0.049,
  "Berlin": 0.046,
  "Schleswig-Holstein": 0.036,
  "Brandenburg": 0.031,
...
const HOUSEHOLD_TARGETS: Record<HouseholdType, number> = {
  alleinlebend: 0.414,
  "paar-ohne-kinder": 0.245,
  "paar-mit-kindern": 0.166,
  alleinerziehend: 0.055,
  mehrpersonen: 0.045,
  rentnerhaushalt: 0.075,
};
const AGE_TARGETS: Record<AgeGroup, number> = { "0-17": 0.184, "18-29": 0.139, "30-44": 0.185, "45-64": 0.276, "65-79": 0.154, "80+": 0.062 };
const EMPLOYMENT_TARGETS: Record<EmploymentStatus, number> = { erwerbstaetig: 0.51, arbeitslos: 0.02, rente: 0.20, bildung: 0.20, "nicht-erwerbstaetig": 0.07 };
...
const SIZE_TARGETS: Record<string, number> = { "1": 0.45, "2": 0.307, "3": 0.122, "4+": 0.121 };
const CHILD_TARGETS: Record<string, number> = { "0": 0.756, "1": 0.116, "2": 0.088, "3+": 0.04 };
const DECILE_TARGETS: Record<string, number> = Object.fromEntries(Array.from({ length: 10 }, (_, index) => [String(index + 1), 0.1]));
...
function householdShape(type: HouseholdType, random: () => number) {
  ...
  if (type === "paar-mit-kindern") return { adults: 2, children: random() < 0.18 ? 3 : random() < 0.58 ? 2 : 1 };
...
function fitShapeToRemaining(type: HouseholdType, shape: { adults: number; children: number }, remaining: number) {
  if (shape.adults + shape.children <= remaining) return { type, ...shape };
  if (remaining === 1) return { type: "alleinlebend" as HouseholdType, adults: 1, children: 0 };
  if (remaining === 2) return { type: "paar-ohne-kinder" as HouseholdType, adults: 2, children: 0 };
  return { type: "paar-mit-kindern" as HouseholdType, adults: 2, children: remaining - 2 };
}
...
function workTime(status: EmploymentStatus, random: () => number): WorkTimeStatus {
  ...
  const draw = random();
  return draw < 0.68 ? "vollzeit" : draw < 0.91 ? "teilzeit" : "geringfuegig";
}
...
function annualIncome(age: number, employment: EmploymentStatus, time: WorkTimeStatus, random: () => number) {
  ...
  const employmentIncome = Math.round(clamp(Math.exp(10.65 + normal(random) * 0.55) * ageFactor * timeFactor, 2_000, 420_000));
  return { employment: employmentIncome, pension: 0, other: Math.round(Math.max(0, Math.exp(7.2 + normal(random) * 1.0) - 1_300)) };
}
...
function rake<T extends Weighted>(items: T[], total: number, dimensions: Array<RakeDimension<T>>) {
  ...
  const initial = total / items.length;
...
function assignIncomeDeciles(persons: SyntheticPerson[]) {
  ...
  const totalWeight = adults.reduce((sum, person) => sum + person.weight, 0);
  let cumulative = 0;
  adults.forEach((person) => {
    const midpoint = cumulative + person.weight / 2;
    person.incomeDecile = Math.min(10, Math.floor(midpoint / Math.max(totalWeight, 1) * 10) + 1);
    cumulative += person.weight;
  });
  persons.filter((person) => person.age < 18).forEach((person) => { person.incomeDecile = 1; });
}

interface CalibrationDimension<T extends Weighted> {
  name: string;
  sourceId: string;
  tolerance: number;
...
function createCalibration(dimensions: Array<CalibrationDimension<SyntheticPerson> | CalibrationDimension<SyntheticHousehold>>): CalibrationEntry[] {
  return dimensions.flatMap((dimension) => {
    const items = dimension.items as Array<SyntheticPerson | SyntheticHousehold>;
    const key = dimension.key as (item: SyntheticPerson | SyntheticHousehold) => string;
    const total = items.reduce((sum, item) => sum + item.weight, 0);
    return Object.entries(dimension.targets).map(([category, target]) => {
      const actual = items.reduce((sum, item) => sum + (key(item) === category ? item.weight : 0), 0) / Math.max(total, 1);
      ...
      return {
        ...
        status: Math.abs(relativeDeviation) <= dimension.tolerance ? "innerhalb-toleranz" as const : "warnung" as const,
...
function validationIssues(households: SyntheticHousehold[], persons: SyntheticPerson[]) {
  ...
  persons.forEach((person) => {
    ...
    group.push(person);
    peopleByHousehold.set(person.householdId, group);
    if (!householdMap.has(person.householdId)) issues.push({ code: "missing-household", severity: "error", entityType: "person", entityId: person.id, message: "Haushaltsreferenz fehlt." });
    ...
    if (person.taxableIncome > person.grossEmploymentIncome + person.otherTaxableIncome + person.pensionIncome + 1) issues.push({ code: "taxable-income", severity: "warning", entityType: "person", entityId: person.id, message: "Steuerbares Einkommen übersteigt die modellierten steuerbaren Bruttoeinkommen." });
  });
  households.forEach((household) => {
    ...
    if (household.householdType === "rentnerhaushalt" && members.some((person) => person.age < 60)) issues.push({ code: "retiree-age", severity: "warning", entityType: "household", entityId: household.id, message: "Rentnerhaushalt enthält eine Person unter 60 Jahren." });
...
function distribution<T>(items: T[], key: (item: T) => string, weight: (item: T) => number): PopulationDistributionItem[] {
  ...
  items.forEach((item) => {
    ...
    grouped.set(category, (grouped.get(category) ?? 0) + weight(item));
  });
...
export function generatePopulation(options: PopulationGenerationOptions): { run: PopulationRun; households: SyntheticHousehold[]; persons: SyntheticPerson[] } {
  ...
  const random = createDeterministicRandom(`${seed}|${sampleSize}|${baselineId}|${POPULATION_MODEL_VERSION}`);
  const households: SyntheticHousehold[] = [];
  const persons: SyntheticPerson[] = [];

  while (persons.length < sampleSize) {
    const householdIndex = households.length;
    const id = `${runId}-h-${String(householdIndex + 1).padStart(6, "0")}`;
    const selectedType = pickWeighted(random, HOUSEHOLD_TARGETS);
    const fitted = fitShapeToRemaining(selectedType, householdShape(selectedType, random), sampleSize - persons.length);
    const householdType = fitted.type;
    ...
    const childAges = Array.from({ length: fitted.children }, () => Math.floor(random() * 18)).sort((a, b) => b - a);
    const memberStart = persons.length;

    for (let adultIndex = 0; adultIndex < fitted.adults; adultIndex += 1) {
      ...
      persons.push({
        id: `${runId}-p-${String(persons.length + 1).padStart(7, "0")}`,
        runId,
        householdId: id,
        age,
        ageGroup: ageGroup(age),
        householdRole: fitted.adults === 1 && fitted.children === 0 ? "alleinlebend" : fitted.children > 0 ? "elternteil" : "partner",
        employmentStatus,
        workTimeStatus,
        grossEmploymentIncome: income.employment,
        otherTaxableIncome: income.other,
        pensionIncome: income.pension,
        transferIncome,
        socialInsuranceIncome: income.employment,
        taxableIncome: Math.round(Math.max(0, income.employment + income.other + income.pension * 0.82 - 2_500)),
        weight: 1,
        federalState,
        communityType,
        incomeDecile: 1,
    ...
    childAges.forEach((age) => persons.push({
      ...
      ageGroup: "0-17",
      householdRole: "kind",
      employmentStatus: "bildung",
      workTimeStatus: "nicht-zutreffend",
      grossEmploymentIncome: 0,
      otherTaxableIncome: 0,
      pensionIncome: 0,
      transferIncome: 3_000,
    ...
    households.push({
      ...
      runId,
      householdType,
      adultCount: fitted.adults,
      childCount: fitted.children,
      childAges,
      ...
      grossIncome,
      disposableIncome,
      transferComponents: { familienleistungen: fitted.children * 3_000, grundsicherung: members.reduce((sum, person) => sum + person.transferIncome, 0) },
      housingStatus,
      grossColdRent: housingStatus === "miete" ? housingCosts : 0,
      housingCosts,
      wealth,
      debt,
      federalState,
  ...
  rake(households, TARGET_HOUSEHOLDS, [
    { categories: HOUSEHOLD_TARGETS, key: (item) => item.householdType },
    { categories: STATE_TARGETS, key: (item) => item.federalState },
    { categories: COMMUNITY_TARGETS, key: (item) => item.communityType },
    { categories: HOUSING_TARGETS, key: (item) => item.housingStatus },
    { categories: SIZE_TARGETS, key: (item) => sizeGroup(item.adultCount + item.childCount) },
    { categories: CHILD_TARGETS, key: (item) => childGroup(item.childCount) },
  ]);
  rake(persons, TARGET_POPULATION, [
    { categories: AGE_TARGETS, key: (item) => item.ageGroup },
    { categories: EMPLOYMENT_TARGETS, key: (item) => item.employmentStatus },
    { categories: STATE_TARGETS, key: (item) => item.federalState },
    { categories: COMMUNITY_TARGETS, key: (item) => item.communityType },
  ]);
  ...
  const calibration = createCalibration([
    { name: "Altersgruppen", sourceId: "source-population-destatis", tolerance: 0.02, targets: AGE_TARGETS, items: persons, key: (item: SyntheticPerson) => item.ageGroup },
    { name: "Haushaltstyp", sourceId: "source-population-microcensus", tolerance: 0.03, targets: HOUSEHOLD_TARGETS, items: households, key: (item: SyntheticHousehold) => item.householdType },
    { name: "Haushaltsgröße", sourceId: "source-population-microcensus", tolerance: 0.04, targets: SIZE_TARGETS, items: households, key: (item: SyntheticHousehold) => sizeGroup(item.adultCount + item.childCount) },
    { name: "Erwerbsstatus", sourceId: "source-population-microcensus", tolerance: 0.03, targets: EMPLOYMENT_TARGETS, items: persons, key: (item: SyntheticPerson) => item.employmentStatus },
    { name: "Einkommensdezil", sourceId: "source-population-model", tolerance: 0.03, targets: DECILE_TARGETS, items: persons.filter((person) => person.age >= 18), key: (item: SyntheticPerson) => String(item.incomeDecile) },
    { name: "Kinderzahl", sourceId: "source-population-microcensus", tolerance: 0.04, targets: CHILD_TARGETS, items: households, key: (item: SyntheticHousehold) => childGroup(item.childCount) },
    { name: "Bundesland", sourceId: "source-population-destatis", tolerance: 0.03, targets: STATE_TARGETS, items: persons, key: (item: SyntheticPerson) => item.federalState },
    ...
    { name: "Wohnen", sourceId: "source-population-microcensus", tolerance: 0.03, targets: HOUSING_TARGETS, items: households, key: (item: SyntheticHousehold) => item.housingStatus },
  ]);
  ...
  const metadata: PopulationRunMetadata = {
    ...
    legalYear: 2026,
    baselineId,
    sourceIds: populationSources.map((source) => source.id),
    sampleSize: persons.length,
    weightedPopulation: persons.reduce((sum, person) => sum + person.weight, 0),
    weightedHouseholds: households.reduce((sum, household) => sum + household.weight, 0),
    calibrationMethod: "Iteratives proportionales Anpassen (40 Raking-Iterationen)",
    quality: {
    ...
    limitations: ["Vollständig synthetisch; keine echten Personen oder amtlichen Mikrodaten.", "Gemeinsame Verteilungen und Vermögen sind modelliert.", "Keine kausale Prognose und keine Regionalisierung unterhalb der Bundesländer."],
    active: true,
  };
...
function queryKey(entity: SyntheticPerson | SyntheticHousehold, groupBy: PopulationQuery["groupBy"]): string {
  ...
  return String((entity as unknown as Record<string, unknown>)[groupBy] ?? "unbekannt");
}
...
export function queryPopulation(runId: string, persons: SyntheticPerson[], households: SyntheticHousehold[], query: PopulationQuery): PopulationQueryResult {
  const entities: Array<SyntheticPerson | SyntheticHousehold> = query.entity === "persons" ? persons : households;
  const groups = new Map<string, Array<SyntheticPerson | SyntheticHousehold>>();
  entities.forEach((entity) => {
    const key = queryKey(entity, query.groupBy);
...
export function estimatePopulationIncomeTax(run: PopulationRunMetadata, persons: SyntheticPerson[], households: SyntheticHousehold[], settings: IncomeTaxSettings, modelLevel: ModelLevel): IncomeTaxResult {
  ...
  const deciles: IncomeTaxDecileResult[] = Array.from({ length: 10 }, (_, index) => {
    const group = units.filter((unit) => unit.decile === index + 1);
    const weight = group.reduce((sum, unit) => sum + unit.household.weight, 0);
    const changes = group.map((unit) => unit.monthlyChange);
    return {
      id: `D${index + 1}`,
      label: `Dezil ${index + 1}`,
      representativeIncome: weight ? group.reduce((sum, unit) => sum + unit.household.grossIncome * unit.household.weight, 0) / weight : 0,
      monthlyChange: weight ? group.reduce((sum, unit) => sum + unit.monthlyChange * unit.household.weight, 0) / weight : 0,
      lowerMonthlyChange: changes.length ? Math.min(...changes) : 0,
      upperMonthlyChange: changes.length ? Math.max(...changes) : 0,
  ...
  return {
    ...
    taxUnitsM: totalUnits / 1_000_000,
    calibrationFactor,
    deciles,
    populationRunId: run.id,
    populationSampleSize: run.sampleSize,
    weightedPopulation: run.weightedPopulation,
    populationDataYear: run.dataYear,
    calibrationStatus: run.quality.status,
  };
}
